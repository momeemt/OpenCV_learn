...
void my_perspective(int argc, char* argv[]) {
    try {
        UMat src, dst;
        Point2f dstPoint[4];
        int xMergin, yMergin;
        ...
        if (argc < 3) {
            throw "few parameter, e.g. <filename> <pattern#>.";
        }
        ...
        switch (pattern) {
            ...
            case 1:
                ...
                dstPoint[1] = srcPoint[1];
                dstPoint[2] = Point(x0 - xMergin, y0 - yMergin);
                dstPoint[3] = Point(x1 - xMergin, y0 + yMergin);
                break;
            ...
            case 2:
                xMergin = src.cols / 6;
                yMergin = src.rows / 6;
                dstPoint[0] = Point(x1 + xMergin, y0 + yMergin);
                dstPoint[1] = srcPoint[1];
                dstPoint[2] = Point(x0 - xMergin, y0 - yMergin);
